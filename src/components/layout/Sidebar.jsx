...
export function MobileSidebar({ open, onClose }) {
  return (
    // AnimatePresence only tracks exit for its DIRECT, keyed motion children.
    // The overlay and the drawer are therefore rendered as two keyed motion
    // elements here — not wrapped in a plain <div> — otherwise the exit never
    // resolves, the full-screen overlay stays mounted over the page after a
    // route change, and navigation appears to do nothing until a refresh.
    <AnimatePresence>
      {open && [
        <motion.div
          key="mobile-overlay"
        ...
        <motion.aside
          ...
          className="glass fixed left-0 top-0 z-[51] flex h-full w-72 transform-gpu flex-col gap-1 rounded-none rounded-r-3xl p-4 lg:hidden"
        >
