...
import {
  ...
  Cpu, Wand2, ScrollText, ListOrdered, Gauge, Network, Bot, Brain, ShieldCheck, FolderTree,
} from 'lucide-react'
import { aiContent } from '@/data/aiContent'
import { aiTopics, aiTopicBySlug, aiGroupById, AI_LEVELS } from '@/data/aiTopics'
...
const LEVEL_TONE = { Beginner: 'text-green-300 bg-green-500/15', Intermediate: 'text-amber-300 bg-amber-500/15', Practical: 'text-violet-300 bg-violet-500/15', Advanced: 'text-red-300 bg-red-500/15' }
...
function buildSections(c) {
  return [
    ...
    { id: 'workflow', label: '🤖 AI Workflow', icon: Bot, color: '#8B5CF6', has: !!c.workflow },
    ...
    { id: 'architecture', label: '🟣 Architecture Diagram', icon: Network, color: '#A855F7', has: !!c.architecture },
    { id: 'visual', label: '🟣 Flowchart / Diagram', icon: Eye, color: '#A855F7', has: !!c.visual },
    { id: 'folders', label: '🟠 Folder Structure', icon: FolderTree, color: '#F97316', has: !!c.folders },
    ...
    { id: 'best', label: '🧠 Best Practices', icon: Brain, color: '#22C55E', has: !!c.best },
    { id: 'security', label: '🔒 Security Considerations', icon: ShieldCheck, color: '#F43F5E', has: !!c.security },
    { id: 'performance', label: '⚡ Performance Notes', icon: Gauge, color: '#F59E0B', has: !!c.performance },
    { id: 'tricky', label: '🔴 Scenario Questions', icon: Wand2, color: '#EF4444', has: !!c.tricky },
    ...
    { id: 'exercises', label: '🟡 Exercises', icon: Dumbbell, color: '#EAB308', has: !!c.exercises },
    { id: 'summary', label: '🟢 Summary / Revision Notes', icon: ListChecks, color: '#22C55E', has: !!c.summary },
    ...
    { id: 'advanced', label: '🚀 Advanced Concepts', icon: Zap, color: '#A855F7', has: !!c.advanced },
...
export default function AiTopicPage() {
  ...
  const meta = aiTopicBySlug[slug]
  const content = aiContent[slug]
  const { completed, bookmarks, toggleComplete, toggleBookmark } = useAiProgress()
  ...
  if (!meta || !content) {
    return (
      <div className="mx-auto max-w-2xl py-20 text-center">
        ...
        <Link to="/learn/ai" className="mt-3 inline-block text-violet-400 hover:underline">← AI Course par wapas jao</Link>
      </div>
    )
  }

  const group = aiGroupById[meta.group]
  const lv = AI_LEVELS[meta.level]
  const idx = aiTopics.findIndex((t) => t.slug === slug)
  const prev = aiTopics[idx - 1]
  const next = aiTopics[idx + 1]
    ...
    <div className="mx-auto flex max-w-6xl gap-8">
      ...
      <article className="min-w-0 flex-1 space-y-10">
        <nav className="flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
          ...
          <Link to="/learn/ai" className="hover:text-slate-300">AI Integration</Link>
          <ChevronRight size={12} />
        ...
        <div className="grid gap-3 border-t border-white/10 pt-8 sm:grid-cols-2">
          {prev ? (
            <Link to={`/learn/ai/${prev.slug}`} className="group flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 transition hover:border-white/20 hover:bg-white/[0.04]">
          ...
          {next && (
            <Link to={`/learn/ai/${next.slug}`} className="group flex items-center justify-end gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-right transition hover:border-white/20 hover:bg-white/[0.04]">
      ...
      <aside className="hidden w-52 shrink-0 xl:block">
        <div className="sticky top-8">
          ...
          <nav className="space-y-1 border-l border-white/10">
            {sections.map((s) => (
              <a key={s.id} href={`#${s.id}`} className="block border-l-2 border-transparent py-1 pl-3 text-xs text-slate-400 transition hover:border-violet-500 hover:text-white">
                {s.label}
              </a>
            ))}
          </nav>
...
function renderSection(id, c) {
  switch (id) {
          ...
          {c.concept.map((t, i) => (
            <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              <h3 className="mb-1.5 flex items-center gap-2 text-sm font-semibold text-white">
                <span className="grid h-5 w-5 place-items-center rounded-md bg-violet-500/15 text-[11px] font-bold text-violet-300">{i + 1}</span>
                <Inline text={t.h} />
              </h3>
    ...
    case 'workflow':
      return <div className="rounded-2xl border border-violet-500/25 bg-violet-500/[0.05] p-5"><Diagram data={c.workflow} /></div>
    ...
    case 'steps':
      return (
        <div className="relative space-y-3 pl-6">
          <div className="absolute left-[7px] top-2 h-[calc(100%-1rem)] w-px bg-gradient-to-b from-violet-500 to-fuchsia-500" />
          {c.steps.map((s, i) => (
            <div key={i} className="relative">
              <span className="absolute -left-[22px] top-0.5 grid h-4 w-4 place-items-center rounded-full bg-violet-500/20 text-[10px] font-bold text-violet-300">{i + 1}</span>
    ...
    case 'architecture':
      return <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5"><Diagram data={c.architecture} /></div>
    ...
    case 'folders':
      return (
        <div className="overflow-hidden rounded-2xl border border-orange-500/20 bg-[#0d1117]">
          {c.folders.note && <div className="border-b border-white/10 bg-white/[0.03] px-4 py-2 text-[11px] font-semibold text-orange-200">{c.folders.note}</div>}
          <pre className="overflow-x-auto p-4 text-[12.5px] leading-relaxed"><code className="font-mono text-slate-300 whitespace-pre">{typeof c.folders === 'string' ? c.folders : c.folders.tree}</code></pre>
        </div>
      )
        ...
        <div className="space-y-4">
          {c.examples.map((ex, i) => (
            <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              ...
              {ex.explain && <p className="mt-3 text-sm leading-relaxed text-slate-400"><span className="text-violet-400">↳ </span><Inline text={ex.explain} /></p>}
            </div>
          ))}
        </div>
      )
    ...
    case 'security':
      return <Callout variant="error" title="🔒 Security Considerations"><Inline text={c.security} /></Callout>
        ...
        <div className="space-y-4">
          {c.tricky.map((t, i) => (
            <div key={i} className="rounded-2xl border border-red-500/20 bg-red-500/[0.04] p-5">
              ...
              {t.explain && <p className="mt-3 text-sm leading-relaxed text-slate-300"><span className="text-violet-400">💡 </span><Inline text={t.explain} /></p>}
            </div>
          ))}
        </div>
      )
    ...
    case 'related':
      return (
        <div className="flex flex-wrap gap-2">
          {c.related.map((s) => {
            const t = aiTopicBySlug[s]
            if (!t) return null
            return (
              <Link key={s} to={`/learn/ai/${s}`} className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2 text-sm text-slate-300 transition hover:border-white/20 hover:text-white">
    ...
    <div className="space-y-2.5">
      {items.map((ex, i) => (
        <div key={i} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4">
          <span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-violet-500/15 text-xs font-bold text-violet-300">{i + 1}</span>
