...
import { Search, Check, Bookmark, ArrowLeft, Bot, ChevronDown } from 'lucide-react'
import { aiGroupedTopics, aiTopics, AI_LEVELS } from '@/data/aiTopics'
import { useAiProgress } from '@/store/useAiProgress'

export function AiSidebarContent({ onNavigate }) {
  useParams()
  ...
  const overall = Math.round((doneCount / aiTopics.length) * 100)

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return aiGroupedTopics
    return aiGroupedTopics
    ...
    <div className="flex h-full flex-col">
      <div className="px-2 pb-4">
        ...
        <div className="flex items-center gap-2.5">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-violet-400 to-fuchsia-500 text-[#0F172A] shadow-lg">
            <Bot size={18} />
          </div>
          <div>
            <p className="text-sm font-bold text-white">AI Integration</p>
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex items-center justify-between text-[11px] text-slate-400">
            <span>Progress</span>
            <span className="font-semibold text-violet-300">{doneCount}/{aiTopics.length} · {overall}%</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.08]">
            <div className="h-full rounded-full bg-gradient-to-r from-violet-400 to-fuchsia-500 transition-all" style={{ width: `${overall}%` }} />
          </div>
        </div>
        ...
        <div className="relative mt-4">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.03] py-2 pl-9 pr-3 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:border-violet-500/50"
          />
        </div>
      </div>
