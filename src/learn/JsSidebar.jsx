...
import { Search, Check, Bookmark, ArrowLeft, Braces, ChevronDown } from 'lucide-react'
import { jsGroupedTopics, jsTopics, JS_LEVELS } from '@/data/jsTopics'
import { useJsProgress } from '@/store/useJsProgress'

export function JsSidebarContent({ onNavigate }) {
  useParams()
  ...
  const overall = Math.round((doneCount / jsTopics.length) * 100)

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return jsGroupedTopics
    return jsGroupedTopics
    ...
    <div className="flex h-full flex-col">
      <div className="px-2 pb-4">
        ...
        <div className="flex items-center gap-2.5">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-amber-400 to-yellow-500 text-black shadow-lg">
            <Braces size={18} />
          </div>
          <div>
            <p className="text-sm font-bold text-white">JavaScript Course</p>
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex items-center justify-between text-[11px] text-slate-400">
            <span>Progress</span>
            <span className="font-semibold text-amber-300">{doneCount}/{jsTopics.length} · {overall}%</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.08]">
            <div className="h-full rounded-full bg-gradient-to-r from-amber-400 to-yellow-500 transition-all" style={{ width: `${overall}%` }} />
          </div>
        </div>
        ...
        <div className="relative mt-4">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.03] py-2 pl-9 pr-3 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:border-amber-500/50"
          />
        </div>
      </div>
