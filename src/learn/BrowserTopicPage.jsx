...
import {
  ...
  Cpu, Brain, Globe, Wand2, ScrollText, ListOrdered, Gauge, Network,
} from 'lucide-react'
import { browserContent } from '@/data/browserContent'
import { browserTopics, browserTopicBySlug, browserGroupById, BROWSER_LEVELS } from '@/data/browserTopics'
...
const LEVEL_TONE = { Beginner: 'text-green-300 bg-green-500/15', Intermediate: 'text-amber-300 bg-amber-500/15', Practical: 'text-orange-300 bg-orange-500/15', Advanced: 'text-red-300 bg-red-500/15' }
...
export default function BrowserTopicPage() {
  const { slug } = useParams()
  const meta = browserTopicBySlug[slug]
  const content = browserContent[slug]
  const { completed, bookmarks, toggleComplete, toggleBookmark } = useBrowserProgress()
  ...
  if (!meta || !content) {
    return (
      <div className="mx-auto max-w-2xl py-20 text-center">
        ...
        <Link to="/learn/browser" className="mt-3 inline-block text-orange-400 hover:underline">← Browser Course par wapas jao</Link>
      </div>
    )
  }

  const group = browserGroupById[meta.group]
  const lv = BROWSER_LEVELS[meta.level]
  const idx = browserTopics.findIndex((t) => t.slug === slug)
  const prev = browserTopics[idx - 1]
  const next = browserTopics[idx + 1]
    ...
    <div className="mx-auto flex max-w-6xl gap-8">
      ...
      <article className="min-w-0 flex-1 space-y-10">
        <nav className="flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
          ...
          <Link to="/learn/browser" className="hover:text-slate-300">Browser</Link>
          <ChevronRight size={12} />
        ...
        <BrowserNotes slug={slug} />

        <div className="grid gap-3 border-t border-white/10 pt-8 sm:grid-cols-2">
          {prev ? (
            <Link to={`/learn/browser/${prev.slug}`} className="group flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 transition hover:border-white/20 hover:bg-white/[0.04]">
          ...
          {next && (
            <Link to={`/learn/browser/${next.slug}`} className="group flex items-center justify-end gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-right transition hover:border-white/20 hover:bg-white/[0.04]">
      ...
      <aside className="hidden w-52 shrink-0 xl:block">
        <div className="sticky top-8">
          ...
          <nav className="space-y-1 border-l border-white/10">
            {sections.map((s) => (
              <a key={s.id} href={`#${s.id}`} className="block border-l-2 border-transparent py-1 pl-3 text-xs text-slate-400 transition hover:border-orange-500 hover:text-white">
                {s.label}
              </a>
            ))}
          </nav>
...
function renderSection(id, c) {
  switch (id) {
          ...
          {c.concept.map((t, i) => (
            <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              <h3 className="mb-1.5 flex items-center gap-2 text-sm font-semibold text-white">
                <span className="grid h-5 w-5 place-items-center rounded-md bg-orange-500/15 text-[11px] font-bold text-orange-300">{i + 1}</span>
                <Inline text={t.h} />
              </h3>
    ...
    case 'steps':
      return (
        <div className="relative space-y-3 pl-6">
          <div className="absolute left-[7px] top-2 h-[calc(100%-1rem)] w-px bg-gradient-to-b from-orange-500 to-rose-500" />
          {c.steps.map((s, i) => (
            <div key={i} className="relative">
              <span className="absolute -left-[22px] top-0.5 grid h-4 w-4 place-items-center rounded-full bg-orange-500/20 text-[10px] font-bold text-orange-300">{i + 1}</span>
        ...
        <div className="space-y-4">
          {c.examples.map((ex, i) => (
            <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
              ...
              {ex.explain && <p className="mt-3 text-sm leading-relaxed text-slate-400"><span className="text-orange-400">↳ </span><Inline text={ex.explain} /></p>}
            </div>
          ))}
        </div>
      )
    ...
    case 'browser':
      return <Callout variant="info" title="Browser mein kaise chalta hai? 🌐"><Inline text={c.browser} /></Callout>
    ...
    case 'performance':
      return <Callout variant="warning" title="⚡ Performance Notes"><Inline text={c.performance} /></Callout>
        ...
        <div className="space-y-4">
          {c.tricky.map((t, i) => (
            <div key={i} className="rounded-2xl border border-red-500/20 bg-red-500/[0.04] p-5">
              ...
              {t.explain && <p className="mt-3 text-sm leading-relaxed text-slate-300"><span className="text-orange-400">💡 </span><Inline text={t.explain} /></p>}
            </div>
          ))}
        </div>
      )
    ...
    case 'related':
      return (
        <div className="flex flex-wrap gap-2">
          {c.related.map((s) => {
            const t = browserTopicBySlug[s]
            if (!t) return null
            return (
              <Link key={s} to={`/learn/browser/${s}`} className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2 text-sm text-slate-300 transition hover:border-white/20 hover:text-white">
    ...
    <div className="space-y-2.5">
      {items.map((ex, i) => (
        <div key={i} className="flex gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4">
          <span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-orange-500/15 text-xs font-bold text-orange-300">{i + 1}</span>
