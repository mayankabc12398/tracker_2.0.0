...
import { Search, Check, Bookmark, ArrowLeft, Palette, ChevronDown } from 'lucide-react'
import { cssGroupedTopics, cssTopics, LEVELS } from '@/data/cssTopics'
import { useCssProgress } from '@/store/useCssProgress'

export function CssSidebarContent({ onNavigate }) {
  ...
  const overall = Math.round((doneCount / cssTopics.length) * 100)

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return cssGroupedTopics
    return cssGroupedTopics
    ...
    <div className="flex h-full flex-col">
      <div className="px-2 pb-4">
        ...
        <div className="flex items-center gap-2.5">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-pink-500 to-purple-500 text-white shadow-lg">
            <Palette size={18} />
          </div>
          <div>
            <p className="text-sm font-bold text-white">CSS Course</p>
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex items-center justify-between text-[11px] text-slate-400">
            <span>Progress</span>
            <span className="font-semibold text-pink-300">{doneCount}/{cssTopics.length} · {overall}%</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.08]">
            <div className="h-full rounded-full bg-gradient-to-r from-pink-500 to-purple-500 transition-all" style={{ width: `${overall}%` }} />
          </div>
        </div>
        ...
        <div className="relative mt-4">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.03] py-2 pl-9 pr-3 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:border-pink-500/50"
          />
        </div>
      </div>
