...
import { Search, Check, Bookmark, ArrowLeft, Globe, ChevronDown } from 'lucide-react'
...
export function BrowserSidebarContent({ onNavigate }) {
  useParams()
  ...
  const overall = Math.round((doneCount / browserTopics.length) * 100)

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return browserGroupedTopics
    return browserGroupedTopics
    ...
    <div className="flex h-full flex-col">
      <div className="px-2 pb-4">
        ...
        <div className="flex items-center gap-2.5">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-orange-400 to-rose-500 text-[#0F172A] shadow-lg">
            <Globe size={18} />
          </div>
          <div>
            <p className="text-sm font-bold text-white">Browser Internals</p>
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex items-center justify-between text-[11px] text-slate-400">
            <span>Progress</span>
            <span className="font-semibold text-orange-300">{doneCount}/{browserTopics.length} · {overall}%</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.08]">
            <div className="h-full rounded-full bg-gradient-to-r from-orange-400 to-rose-500 transition-all" style={{ width: `${overall}%` }} />
          </div>
        </div>
        ...
        <div className="relative mt-4">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/10 bg-white/[0.03] py-2 pl-9 pr-3 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:border-orange-500/50"
          />
        </div>
      </div>
