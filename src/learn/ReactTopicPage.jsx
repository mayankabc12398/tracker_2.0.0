...
import {
  ...
  Cpu, Brain, Globe, Wand2, ScrollText, ListOrdered, Gauge,
} from 'lucide-react'
import { reactContent } from '@/data/reactContent'
import { reactTopics, reactTopicBySlug, reactGroupById, REACT_LEVELS } from '@/data/reactTopics'
...
const LEVEL_TONE = { Beginner: 'text-green-300 bg-green-500/15', Intermediate: 'text-amber-300 bg-amber-500/15', Practical: 'text-cyan-300 bg-cyan-500/15', Advanced: 'text-red-300 bg-red-500/15' }
...
function buildSections(c) {
  return [
    ...
    { id: 'steps', label: '🔵 Step-by-Step Working', icon: ListOrdered, color: '#06B6D4', has: !!c.steps },
    { id: 'internals', label: '🟣 Internal Behavior', icon: Cpu, color: '#A855F7', has: !!c.internals },
    { id: 'visual', label: '🟣 Visual / Flowchart', icon: Eye, color: '#A855F7', has: !!c.visual },
...
export default function ReactTopicPage() {
  ...
  const meta = reactTopicBySlug[slug]
  const content = reactContent[slug]
  const { completed, bookmarks, toggleComplete, toggleBookmark } = useReactProgress()
  ...
  if (!meta || !content) {
    return (
      <div className="mx-auto max-w-2xl py-20 text-center">
        ...
        <Link to="/learn/react" className="mt-3 inline-block text-cyan-400 hover:underline">← React Course par wapas jao</Link>
      </div>
    )
  }

  const group = reactGroupById[meta.group]
  const lv = REACT_LEVELS[meta.level]
  const idx = reactTopics.findIndex((t) => t.slug === slug)
  const prev = reactTopics[idx - 1]
  const next = reactTopics[idx + 1]
    ...
    <div className="mx-auto flex max-w-6xl gap-8">
      ...
      <article className="min-w-0 flex-1 space-y-10">
        <nav className="flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
          ...
          <Link to="/learn/react" className="hover:text-slate-300">React</Link>
          <ChevronRight size={12} />
        ...
        <ReactNotes slug={slug} />

        <div className="grid gap-3 border-t border-white/10 pt-8 sm:grid-cols-2">
          {prev ? (
            <Link to={`/learn/react/${prev.slug}`} className="group flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 transition hover:border-white/20 hover:bg-white/[0.04]">
          ...
          {next && (
            <Link to={`/learn/react/${next.slug}`} className="group flex items-center justify-end gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-right transition hover:border-white/20 hover:bg-white/[0.04]">
    ...
    case 'steps':
      return (
        <div className="relative space-y-3 pl-6">
          <div className="absolute left-[7px] top-2 h-[calc(100%-1rem)] w-px bg-gradient-to-b from-cyan-500 to-sky-500" />
          {c.steps.map((s, i) => (
    ...
    case 'related':
      return (
        <div className="flex flex-wrap gap-2">
          {c.related.map((s) => {
            const t = reactTopicBySlug[s]
            if (!t) return null
            return (
              <Link key={s} to={`/learn/react/${s}`} className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.02] px-3 py-2 text-sm text-slate-300 transition hover:border-white/20 hover:text-white">
