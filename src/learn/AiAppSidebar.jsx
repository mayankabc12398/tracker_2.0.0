...
import { Search, Check, Bookmark, ArrowLeft, Rocket, ChevronDown } from 'lucide-react'
...
export function AiAppSidebarContent({ onNavigate }) {
  useParams()
  ...
  const overall = Math.round((doneCount / aiAppTopics.length) * 100)

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase()
    if (!q) return aiAppGroupedTopics
    return aiAppGroupedTopics
    ...
    <div className="flex h-full flex-col">
      <div className="px-2 pb-4">
        ...
        <div className="flex items-center gap-2.5">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-cyan-400 to-blue-500 text-[#0F172A] shadow-lg">
            <Rocket size={18} />
          </div>
          <div>
            <p className="text-sm font-bold text-white">AI App Engineering</p>
            <p className="-mt-0.5 text-[11px] text-slate-500">Build · Optimize · Ship</p>
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex items-center justify-between text-[11px] text-slate-400">
            <span>Progress</span>
            <span className="font-semibold text-cyan-300">{doneCount}/{aiAppTopics.length} · {overall}%</span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-white/[0.08]">
            <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 transition-all" style={{ width: `${overall}%` }} />
          </div>
        </div>
