...
import { LearnSidebarContent } from './LearnSidebar'

export function LearnLayout() {
  const [open, setOpen] = useState(false)

  return (
    // Self-contained dark docs surface (#0F172A) regardless of app theme.
    <div className="min-h-screen bg-[#0F172A] text-slate-200">
      {/* Ambient gradient glow */}
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -left-40 -top-40 h-96 w-96 rounded-full bg-indigo-600/10 blur-[120px]" />
        <div className="absolute -right-40 top-20 h-96 w-96 rounded-full bg-purple-600/10 blur-[120px]" />
      </div>
      ...
      <div className="relative mx-auto flex max-w-[1400px]">
        ...
        <div className="fixed inset-x-0 top-0 z-40 flex items-center gap-3 border-b border-white/5 bg-[#0F172A]/90 px-4 py-3 backdrop-blur-xl lg:hidden">
          <button onClick={() => setOpen(true)} className="grid h-9 w-9 place-items-center rounded-xl text-slate-300 hover:bg-white/5">
            <Menu size={20} />
          </button>
          <span className="text-sm font-semibold text-white">HTML Course</span>
        </div>
          ...
          {open && (
            <div className="fixed inset-0 z-50 lg:hidden">
              ...
              <motion.aside
                ...
                className="absolute left-0 top-0 h-full w-80 border-r border-white/10 bg-[#0F172A] p-4"
              >
                <button onClick={() => setOpen(false)} className="absolute right-3 top-3 text-slate-400 hover:text-white">
                  <X size={20} />
                </button>
                <LearnSidebarContent onNavigate={() => setOpen(false)} />
              </motion.aside>
            </div>
          )}
        </AnimatePresence>
